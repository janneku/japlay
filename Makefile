CC = gcc
CFLAGS = -O2 -W -Wall `pkg-config ao glib-2.0 --cflags`
LDFLAGS = `pkg-config ao glib-2.0 gthread-2.0 --libs`
PLUGIN_CFLAGS = $(CFLAGS) -fPIC
PLUGIN_LDFLAGS = $(LDCFLAGS) -shared

OBJ = main.o utils.o playlist.o iowatch.o unixsocket.o
...
ui_gtk.o:	ui_gtk.c
	$(CC) $(PLUGIN_CFLAGS) `pkg-config gtk+-2.0 --cflags` -c $<

%.o:	%.c
	$(CC) $(CFLAGS) -c $<

$(GTK_BINARY):	$(OBJ) ui_gtk.o
	$(CC) $(OBJ) ui_gtk.o -o $@ $(LDFLAGS) `pkg-config gtk+-2.0 --libs`
